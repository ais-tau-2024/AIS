	...
	<div class="storage-view">
		...
		<div class="storage-section p-4">
			...
			<div v-else id="workspace-page">
				...
				<ul class="file-list">
					<li v-for="dir in directories" :key="dir" draggable="true"
                        ...
                        <button class="btn btn-danger" @click="deleteFolder(dir)">Удалить</button>
                        </div>
                    </li>
					<li
						v-for="file in files"
					...
					>
						...
						<div class="file-actions" @click.stop>
							<button class="btn btn-primary" @click="openFile(file)">Открыть</button>
							<button class="btn btn-success" @click="downloadFile(file)">Скачать</button>
							<button class="btn btn-danger" @click="deleteFile(file)">Удалить</button>
						</div>
					</li>
				</ul>
				<div class="actions">
			...
			<div v-if="showGrantAccessModal" class="modal">
				<div class="modal-content w-50 h-50">
					<h3 class="fs-3 m-0 p-0">Предоставить доступ</h3>
					<hr />
					{{ selectedTeacher }}
					...
					<div>
						<button @click="grantAccess" class="btn btn-success">
							Предоставить
						</button>
						<button
			...
			>
				<div class="file-viewer-content">
					<button class="file-viewer-close" @click="closeFileModal">
						X
					</button>
					<div class="content">
...
export default {
	...
	methods: {
		addNotification({notification, status}={}) {
			this.$refs.notifier.addNotification(notification, status);
		},
		...
		async fetchAvailableTeachers() {
			try {
				const res = await axios.get(
					`${axiosDefaultsBaseURL}/teachers/available/`,
					{
		...
		async grantAccess() {
			...
			try {
				...
				await axios.post(
					`${axiosDefaultsBaseURL}/desktops/${this.currentWorkspace}/grant/`,
		...
		async fetchDesktops() {
			try {
				const res = await axios.get(
					axiosDefaultsBaseURL + "/desktops/list/",
					{
						headers: {
							auth: localStorage.getItem("ais.auth.token"),
						},
		...
		async deleteWorkspace(desktopId) {
			if (confirm("Удалить рабочий стол?")) {
				try {
					await axios.delete(
						axiosDefaultsBaseURL + `/desktops/${desktopId}/`,
						{
							headers: {
								auth: localStorage.getItem("ais.auth.token"),
							},
		...
		async renameWorkspace(desktopId, oldName) {
			const newName = prompt(
				"Введите новое название рабочего стола",
				oldName,
			);
			...
			try {
				const res = await axios.get(
					axiosDefaultsBaseURL +
						`/desktops/${this.currentWorkspace}/files/`,
					{
        ...
        async deleteFolder(folder) {
            if (confirm("Удалить папку? Все содержимое будет удалено!")) {
                try {
                await axios.delete(
                    `${axiosDefaultsBaseURL}/desktops/${this.currentWorkspace}/file/`,
                    {
                    data: { path: `${this.currentPath ? this.currentPath + "/" : ""}${folder}` },
		...
		async createFolder() {
			...
			if (folderName) {
				...
				try {
					await axios.post(
						axiosDefaultsBaseURL +
							`/desktops/${this.currentWorkspace}/createFolder/`,
						{ path },
						{
							headers: {
								auth: localStorage.getItem("ais.auth.token"),
        ...
        handleDropOnBreadcrumb(destinationPath, event) {
            ...
            axios.put(`${axiosDefaultsBaseURL}/desktops/${this.currentWorkspace}/file/`, {
