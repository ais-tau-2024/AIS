						...
						<div class="form-group">
							...
							<div class="input-group">
								...
								<input
									...
									disabled
									:value="
										transliterateToLatin(data.firstName)
									"
								/>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="input-group">
								...
								<input
									...
									disabled
									:value="
										transliterateToLatin(data.patronymic)
									"
								/>
							</div>
						</div>
					...
					<div class="form-group">
						<div class="input-group">
							...
							<Multiselect
								...
								@search-change="
									fetchAutoComplete($event, 'birthPlace')
								"
							/>
						</div>
					</div>
					...
					<div class="form-group">
						<div class="input-group">
							...
							<Multiselect
								...
								placeholder="Введите название"
								@search-change="
									fetchAutoComplete($event, 'residencePlace')
								"
							/>
						</div>
						<div class="input-group">
							...
							<input
								type="text"
								v-model="data.residentialAddress"
							/>
						</div>
					</div>
...
export default {
	...
	data() {
		return {
			...
			nationalities: ["Казахи", "Русские"],
			genders: ["male", "female"],  // Мужский, Женский
	...
	async created() {
		...
		if (auth) {
			...
			me.placeOfBirth = me.placeOfBirth
				? typeof me.placeOfBirth === "object" &&
					"label" in me.placeOfBirth
					? me.placeOfBirth
					: { label: me.placeOfBirth }
				: null;

			me.registrationPlace = me.registrationPlace
				? typeof me.registrationPlace === "object" &&
					"label" in me.registrationPlace
					? me.registrationPlace
					: { label: me.registrationPlace }
				: null;

			me.residencePlace = me.residencePlace
				? typeof me.residencePlace === "object" &&
					"label" in me.residencePlace
					? me.residencePlace
	...
	methods: {
		...
		fetchAutoComplete(searchTerm, field) {
			...
			axios
				.get(
					`${axios.defaults.baseURL}/helper/autocomplete/?term=${uri}`,
				)
				.then((response) => {
